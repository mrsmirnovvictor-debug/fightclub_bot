...
async def walk_at(page):
    return await page.evaluate("walk && [Math.round(walk.x), Math.round(walk.y)]")


async def tap_street(page, fx, fy):
    """Ткнуть по картинке в долях от неё — как пальцем по асфальту."""
    box = await page.locator(".walk-svg").bounding_box()
    await page.mouse.click(box["x"] + box["width"] * fx,
                           box["y"] + box["height"] * fy)


async def test_the_marked_district_lets_you_walk_it(server):
    """Центр размечен: на нём есть асфальт, боец и завсегдатаи."""
    async with async_playwright() as pw:
        browser, page = await open_map(pw, server)
        await page.wait_for_selector("#walk-me")

        assert await page.locator(".walk-floor").count() == 1
        # Трое завсегдатаев и сам боец
        assert await page.locator(".walk-figure").count() == 4
        names = await page.locator(".walk-local .walk-name").all_text_contents()
        assert "Вышибала" in names
        await browser.close()


async def test_an_unmarked_district_opens_the_old_way(server):
    """Размечен пока один район. Остальные открываются нажатием на дверь."""
    async with async_playwright() as pw:
        browser, page = await open_map(pw, server, city_map("bank"))
        await page.wait_for_selector(".zone-house")

        assert await page.locator(".walk-figure").count() == 0
        assert await page.locator(".walk-floor").count() == 0
        # А двери на месте и нажимаются
        assert await page.locator(".zone-touch").count() == 3
        await browser.close()


async def test_you_cannot_walk_a_district_you_only_look_at(server):
    """Чужой район листают глазами: ноги за картинкой не ходят."""
    async with async_playwright() as pw:
        browser, page = await open_map(pw, server, city_map("bank"))
        await page.wait_for_selector(".zone-house")

        # Ушли смотреть Центр, стоя при этом в Деловом квартале
        await page.evaluate("mapShown = 'main_hub'; paintDistrict()")
        await page.wait_for_timeout(200)

        assert await page.locator(".walk-figure").count() == 0, "ходит не там, где стоит"
        await browser.close()


async def test_a_tap_on_the_street_takes_you_there(server):
    """Ткнул по асфальту — боец пошёл."""
    async with async_playwright() as pw:
        browser, page = await open_map(pw, server)
        await page.wait_for_selector("#walk-me")
        before = await walk_at(page)
        stood = await page.locator("#walk-me").get_attribute("transform")

        await tap_street(page, 0.45, 0.85)
        await page.wait_for_timeout(3500)

        after = await walk_at(page)
        assert after != before, "боец не сдвинулся"
        assert after[1] > before[1], "пошёл не туда, куда ткнули"
        # И фигурка на экране уехала вместе с ним, а не осталась стоять
        assert await page.locator("#walk-me").get_attribute("transform") != stood
        await browser.close()


async def test_a_tap_outside_the_street_is_ignored(server):
    """По нарисованному дому не ходят: туда нажимать бесполезно."""
    async with async_playwright() as pw:
        browser, page = await open_map(pw, server)
        await page.wait_for_selector("#walk-me")
        before = await walk_at(page)

        await tap_street(page, 0.5, 0.2)  # стена клуба
        await page.wait_for_timeout(900)

        assert await walk_at(page) == before
        await browser.close()


async def test_the_walk_turns_the_corner_by_itself(server):
    """Упёрся в угол — скользит вдоль стены, а не встаёт намертво.

    Без этого до соседнего рукава улицы бойца пришлось бы вести двумя
    нажатиями: он шёл бы по прямой, упирался в дом и останавливался.
    """
    async with async_playwright() as pw:
        browser, page = await open_map(pw, server)
        await page.wait_for_selector("#walk-me")

        await tap_street(page, 0.45, 0.85)   # вниз по полосе
        await page.wait_for_timeout(3500)
        await tap_street(page, 0.79, 0.69)   # за угол, к мастерской
        await page.wait_for_timeout(5000)

        where = await walk_at(page)
        assert where[0] > 700, f"за угол не вышел: {where}"
        await browser.close()


async def test_the_door_lights_up_when_you_reach_it(server):
    """Подошёл вплотную — дверь загорелась: видно, что можно входить."""
    async with async_playwright() as pw:
        browser, page = await open_map(pw, server)
        await page.wait_for_selector("#walk-me")

        # Вышел из клуба — его дверь рядом, она и горит
        assert await page.locator(".zone-house.reached").count() == 1
        await tap_street(page, 0.45, 0.85)
        await page.wait_for_timeout(3500)

        assert await page.locator(".zone-house.reached").count() == 0, "горит издали"
        await browser.close()


async def test_a_local_speaks_only_up_close(server):
    """Через всю улицу с завсегдатаем не поговоришь — к нему идут."""
    async with async_playwright() as pw:
        browser, page = await open_map(pw, server)
        await page.wait_for_selector("#walk-me")
        said = []
        page.on("dialog", lambda one: said.append(one.message) or
                asyncio.ensure_future(one.dismiss()))

        # Разносчик стоит внизу полосы, боец — у клуба
        newsboy = page.locator(".walk-local").filter(has_text="Разносчик")
        await newsboy.click()
        await page.wait_for_timeout(600)
        assert not said, "заговорил, не сходя с места"

        # Зато пошёл к нему — и вблизи отвечает
        await page.wait_for_timeout(4000)
        await newsboy.click()
        await page.wait_for_timeout(400)
        assert said and "подвале" in said[0]
        await browser.close()


async def test_the_marking_tool_is_not_a_game_feature(server):
    """Режим разметки живёт только по прямой просьбе в адресе."""
    async with async_playwright() as pw:
        browser, page = await open_map(pw, server)
        await page.wait_for_selector("#walk-me")

        assert await page.locator("#tune").count() == 0
        await browser.close()
