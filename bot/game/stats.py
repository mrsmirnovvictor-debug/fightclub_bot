...
# ---------- потолки временно сняты ----------
#
# Хозяин клуба попросил посмотреть бой без ограничений. Пока здесь True,
# потолки шансов стоят на 100% — арифметика прежняя, просто min() больше не
# срабатывает. Вернуть всё как было — поставить False: прежние числа стоят
# рядом, в тех же строках.
#
# Потолки урона это не трогает: сопротивление и броня режут не шанс, а сам
# удар, и на 100% бой встал бы — каждое попадание доходило бы единицей.
NO_LIMITS = True
...
MAX_DODGE_CHANCE = 1.0 if NO_LIMITS else 0.55
