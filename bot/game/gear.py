...
# Точность, уворот, крит и антикрит вещи дают долями. Основные доли живут
# на оружии и идут лестницей: 10–15% на пятом уровне, 45–50% на десятом —
# по семь пунктов за уровень. Числа намеренно крупные: в бою они
# вычитаются парами (точность сбивает уворот, антикрит — крит), поэтому
# решает не величина сама по себе, а разница внутри пары. Мелкая доля
# тонула в характеристиках и не меняла ничего.
#
# Отсюда и правило пары: выросла одна сторона — вторая обязана вырасти
# так же. Оружие ассасина поэтому несёт не только крит, но и точность:
# без неё он не догонит уворот трикстера, который вырос вместе с ним.
# Одежда процентами не торгует: её дело — броня и запас здоровья. Доля на
# ней остаётся маленькой, иначе комплект из восьми вещей перевесит оружие
# любой ступени — их проценты складываются, а оружие всего одно.
EARLY_LEVELS = 5
EARLY_SHARE_CAP = 0.05
LATE_SHARE_CAP = 0.10

WEAPON_SHARE_ANCHOR = 5  # уровень, на котором полоса известна точно
WEAPON_SHARE_FLOOR = 0.10  # нижний край полосы на этом уровне
WEAPON_SHARE_TOP = 0.15  # верхний
WEAPON_SHARE_STEP = 0.07  # насколько полоса едет вверх за уровень


def weapon_share_cap(level: int) -> float:
    """Верх полосы процентов для оружия этого уровня.

    Ниже пятого уровня лестница упирается в свой пол: на первых ступенях
    бойцу важнее научиться бить, чем считать доли.
    """
    step = WEAPON_SHARE_TOP + WEAPON_SHARE_STEP * (level - WEAPON_SHARE_ANCHOR)
    return round(max(EARLY_SHARE_CAP, step), 2)


def weapon_share_floor(level: int) -> float:
    """Низ полосы: оружие своей ступени обязано заметно прибавлять."""
    step = WEAPON_SHARE_FLOOR + WEAPON_SHARE_STEP * (level - WEAPON_SHARE_ANCHOR)
    return round(max(0.0, step), 2)
