...
# контрудар, не отыгрывая до девятого уровня. Именно поэтому они не товар,
# а награда: на прилавок такие числа не выкладывают, иначе они решают
# исход боя за игрока и ломают баланс классов. Их выдают руками — тем,
# кто гоняет бой на стенде. Обычные копии тех же вещей (bandana, wraps,
# sneakers, wife_beater) остались на прилавке — с числами по потолку.
TEST_GEAR: tuple[str, ...] = (
    "test_bandana",
    "test_wraps",
    "test_sneakers",
...
# Набор ассасина: обычный товар на прилавке с пятого уровня, за кредиты.
# Числа у него в рамках потолка — это содержимое лавки, а не тестовая выдача.
...
# Всё, что выдаётся руками, а не покупается: на прилавке этих вещей нет
# (`reward=True`), поэтому ни в витрину, ни в эталонный комплект, ни в
# баланс они не попадают.
GRANTED_GEAR: tuple[str, ...] = TEST_GEAR
