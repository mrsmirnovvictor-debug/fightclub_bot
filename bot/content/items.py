...
`tests/test_content.py`: полоса процентов, лестница урона и плоских
прибавок, выносливости на вещах нет. Новая вещь проходит этот тест —
значит, она не ломает баланс классов.

Проценты живут на оружии и идут полосой: 10–15% на пятом уровне, 45–50%
на десятом. Каждое оружие несёт свою долю и ответ той паре, которой его
класс держит удар: уворот сбивается точностью, крит — антикритом, и в бою
они вычитаются. Нападение берёт низ полосы, ответ — верх: у нападения уже
есть фора от характеристики класса.
...
ITEMS: tuple[Item, ...] = (
    ...
    # Кувалда Босса Казино. Не товар и не награда игроку: её носит один
    # боец на весь клуб, и купить её нельзя. Числа нарочно не похожи на
    # прилавочные — медленная и сокрушительная: замах широкий, точность
    # низкая. Обычная кувалда с прилавка бьёт на 8–10 и ровно, а эта
    # редко доходит и почти сносит, когда доходит. Босс, одетый по
    # прилавку, проигрывал одетому игроку — теперь у него есть своё
    Item(
        "boss_sledge",
        "Кувалда Босса",
        Slot.WEAPON,
        ...
        instrumental="кувалдой",
        strength=3,
        # Средний урон — ровно десятой ступени (у биты 22–28, тут 10–40), а
        # «медленная и сокрушительная» живёт в разбросе, а не в слабой
        # строке. Точность и антикрит взяты по лестнице: она у танкового
        # оружия ровная, по 0.07 за ступень, и выбиваться из неё нечему
        damage_min=10,
        damage_max=40,
        accuracy=0.50,
        anticrit=0.50,
        level_required=10,
        requires=Stats(endurance=26),
        reward=True,
        for_classes=(TANK,),
    ),
    # Остальной комплект Босса. Числа скопированы с прилавочных вещей,
    # которые он носил раньше: менялась не сила, а вид. Свои коды нужны
    # ради картинок — общий код означал бы общую картинку, и новый арт
    # перекрасил бы мотошлем половине клуба
    Item(
        "boss_helmet",
        "Шлем Босса",
        Slot.HEAD,
    ...
    # ---------- пятый уровень: шулер, вышибала и кулачный боец ----------
    #
    # Три набора на одну ступень: у трикстера трость и карманы, у танка
    # броня и кувалда, у воина дубинка и штурмовые брюки. Ассасин на этой
    # ступени донашивает своё: его клинок приходит с подпиской, и в
    # эталонный комплект (а значит, и в балансную матрицу) не попадает —
    # оттого на пятом уровне танк держит его увереннее обычного.
    #
    # Доли на оружии — по полосе своей ступени: нападение по низу, ответ
    # по верху. Трость шулера уворачивается и не подставляется под крит,
    # кувалда бьёт точно и терпит крит, дубинка точная и тоже терпит.
    Item(
        "cardsharp_cane",
        "Трость шулера",
        Slot.WEAPON,
    ...
    Item(
        ...
        anticrit=0.15,
        level_required=5,
        requires=Stats(strength=13, endurance=14),
        price=170,
        for_classes=(TANK,),
    ),
    Item(
        "bouncer_armor_jacket",
        "Бронекуртка вышибалы",
        Slot.JACKET,
        "🧥",
        hp=18,
        armor_min=4,
    ...
    # этом держатся в тех же границах, что и витрина клуба: +4 плоских,
    # 10% на одежде и полоса 45–50% на оружии десятого уровня.
    #
