...
def help_text(turn_timeout: int = 30, round_break: int = 30) -> str:
    return (
        ...
        "/updates — отметить ветку доской объявлений клуба (админы)\n"
        "/duel — вызов на кулаках (бой с оружием идёт в карточке)\n"
        "/battle 3 5-8 — собрать командный бой трое на трое для уровней 5–8\n"
        "/royale 6 — собрать королевскую битву: каждый сам за себя\n"
        ...
        "🥊 В ветке дерутся только на кулаках: вещи остаются в раздевалке, "
        "спорят чистые характеристики, и снашивать нечего. Кнопки там у всех "
        "одни и те же — пять зон удара и пять блоков.\n"
        "⚔️ Бои с оружием, групповые, рейды и турниры идут в карточке, "
        "вкладка «Клуб»: там дерутся в том, что надето, вещи снашиваются, а "
        "набор кнопок у каждого свой — второе оружие даёт второй удар, щит "
        "расширяет блок до трёх зон.\n"
        "Ринг с оружием (/arena_gear) остаётся: он задаёт режим групповым боям "
        "и турнирам, которые собирают в этой ветке.\n\n"
        ...
        "время сбора — дерутся тем составом, что успел прийти. Состав можно "
        "собрать и не выходя из карточки: вкладка «Клуб», раздел «Отряд». "
        "Сам бой идёт там же.\n"
        ...
        "Пары дерутся по очереди: бот объявляет их в ветке, а сам бой идёт в "
        "карточке. Перед каждым боем бот лечит "
        ...
        "Панель хода: слева удары по зонам, справа блоки. Класс на удары и "
        "блоки не влияет: разница между классами живёт в характеристиках. "
        "Соперник твоих нажатий не видит.\n"
        "На кулаках у всех одно и то же: один удар, один блок на две смежные "
        "зоны. В карточке набор зависит от снаряжения — второе оружие даёт "
        "второй столбец ударов, щит закрывает блоком три зоны. Без оружия бьют "
        "кулаком, с оружием — кастетом, ножом и чем найдёшь.\n"
        ...
        "с 4-го — оружие, с 5-го верхняя одежда, штаны, наручи и пояса, с 6-го "
        "новое оружие, головные уборы и обувь, с 7-го оружие и перчатки, "
        "с 8-го последнее оружие. В каждой партии есть вещь под "
        ...
        "(кепка на голову, футболка и верхняя одежда на корпус и живот, пояс на "
        "пояс, штаны на пояс и ноги, обувь на ноги), плюс проценты точности "
        "и антикрита и запас здоровья. Футболка и верхняя одежда лежат слоями: "
        "их броня складывается.\n\n"
        "<b>Эликсиры</b>\n"
